...
RSpec.describe "Inoculate::Manufacturer" do
  ...
  context "builders" do
    ...
    context "transients" do
      ...
      it "does not allow nil blocks" do
        expect { subject.transient(:service) }.to raise_error Inoculate::Errors::RequiresCallable
      end
    end
  end
  ...
  context "building" do
    ...
    subject(:accessor_module) { manufacturer.send(lifecycle, name) { factory_spy.new }[:accessor_module] }
    let(:lifecycle) { :transient }
    let(:name) { :service }
    let(:factory_spy) { spy("object") }
    ...
    context "transients" do
      ...
      it "creates a new object instance every call" do
        ...
        expect(factory_spy).to have_received(:new).twice
      end
    end
  end
end
