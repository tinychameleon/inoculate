...
RSpec.describe "Inoculate::Manufacturer" do
  ...
  context "builders" do
    context "registering" do
      [:transient, :instance, :singleton, :thread_singleton].each do |lifecycle|
    ...
    RSpec.shared_examples "life cycle checks" do |lifecycle|
      ...
      it "can be registered with a block" do
        subject.send(lifecycle, :service) {}
      ...
      it "does not allow other names" do
        expect { subject.send(lifecycle, 1, &-> {}) }.to raise_error Inoculate::Errors::InvalidName
      end

      it "does not allow names which can't be attr_reader parameters" do
        expect { subject.send(lifecycle, :@service, &-> {}) }.to raise_error Inoculate::Errors::InvalidName
      end

      it "does not allow nil blocks" do
        expect { subject.send(lifecycle, :service) }.to raise_error Inoculate::Errors::RequiresCallable
      end
    end
